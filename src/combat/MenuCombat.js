...
function MenuCombat(player, combat){
	this.player = player;
...
MenuCombat.prototype.afficheToi=function(){
	...
	switch(this.mode){
		...
		case(1):	//menu attaque
			...
			context.fillText(this.player.dresseur.getPokemon(0).getAttaqueNum(0).getName(),390,330,270);
			context.fillText(this.player.dresseur.getPokemon(0).getAttaqueNum(1).getName(),390,400,270);
			context.fillText(this.player.dresseur.getPokemon(0).getAttaqueNum(2).getName(),390,470,270);
			context.fillText(this.player.dresseur.getPokemon(0).getAttaqueNum(3).getName(),390,540,270);
		...
		case(MenuCombatMode.pokemons):
			...
			context.fillStyle=Color.Black;
			for(var i = 0;i<this.player.dresseur.pokemons.length;i++){
	...
	switch(this.mode){
		...
		case MenuCombatMode.objets:
			switch(touche){
				case(BUTTON.DOWN):
					this.selectionObjet++;
					if(this.selectionObjet>4){			//this.player.inveentaire.length +1 (pour le bouton retour )
						this.selectionObjet = 0;
					}
				break;
				case(BUTTON.UP):
...
MenuCombat.prototype.valide=function(){
	...
	case MenuCombatMode.pokemons:
		if(this.selectionPokemon != this.player.dresseur.pokemons.length){
			this.player.dresseur.echange(this.player.dresseur.getPokemon(this.selectionPokemon),this.player.dresseur.getPokemon(0));
			this.mode = MenuCombatMode.global;
			this.combat.mode = CombatMode.attaque;
			this.player.dresseur.attaqueCanceled = true;
		}
